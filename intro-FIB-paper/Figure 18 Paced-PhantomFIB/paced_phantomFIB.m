...
% Parameters
dt = 0.1;       % time step
terminaltime = 3000; % simulation time in ms
...
cellType2 = struct('a', 0.01,    'b', 5.,   'c', 1.0, 'epsilon', .0040, 'sigma', 0.6, 'g_horz', .027, 'g_vert', .015); % Slow Recovery
cellType3 = struct('a', 0.02,    'b', 2.9,   'c', 1.0, 'epsilon', .007, 'sigma', 1.6, 'g_horz', .027, 'g_vert', .015); % Fast Recovery
...
interv = 200;
% External stimulus
I_ext = @(t) (4 .* (mod(t,interv) < 50))*(t<1600) ;% + 4 .* (t>180)*(t<181);
...
if GenerateHeatMap == 1
    % --- User options ---
    desiredTimeSpacing  = 20; % in milliseconds (or same unit as time_steps)
