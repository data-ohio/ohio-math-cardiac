...
cellType2 = struct('a', 0.01,    'b', 5.5,   'c', 1.0, 'epsilon', .0040, 'sigma', 0.6, 'g_horz', .027, 'g_vert', .015); % Slow Recovery
cellType3 = struct('a', 0.02,    'b', 2.9,   'c', 1.0, 'epsilon', .007, 'sigma', 1.6, 'g_horz', .027, 'g_vert', .015); % Fast Recovery

% Scar Tissue Parameters
...
function [dv, dw] = derivatives(v, w, ...
    ...
    % Left and Right Boundaries Insulated
    left   = [v(:,1), v(:,1:end-1)];
    right  = [v(:,2:end), v(:,end)];
